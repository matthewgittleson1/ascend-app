...
const options = [
  'ASAP (Aggressive approach)',
  'Fast (Standard)',
  'Steady (Long term)'
];

export default function QuizResultsSpeedScreen() {
  const router = useRouter();
  const { updateUserData } = useOnboarding();

  const handleSelect = (resultsSpeed: string) => {
    updateUserData({ resultsSpeed });
    router.push('/onboarding/benefits-1');
  };
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.content}>
          <Text style={styles.question}>How quickly do you want to see results?</Text>
